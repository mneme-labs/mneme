...
MnemeCache speaks a binary wire protocol over TLS 1.3. The reference client is
the Rust **Pontus** crate (included). Community clients for Python, Node.js,
Go, and Java implement the same protocol.

See [docs/clients/README.md](clients/README.md) for detailed client
documentation.
...
Install: `pip install mneme-client`

```python
import asyncio
from mneme import MnemeClient, Consistency, PoolConfig

async def main():
    # -- Connection pool (production) --
    pool = await MnemeClient.pool(PoolConfig(
        addr="10.0.0.1:6379",
    ...
    # -- Strings --
    await conn.set("greeting", "hello world")
    await conn.set("session:abc", "token123", ttl=3600)
    val = await conn.get("greeting")                      # "hello world"
    await conn.del_keys(["greeting", "session:abc"])

    # -- Counters --
    await conn.incr("page:views")
    await conn.incrby("page:views", 10)

    # -- Hashes --
    await conn.hset("user:1001", {"name": "Alice", "email": "alice@example.com"})
    name = await conn.hget("user:1001", "name")

    # -- Lists --
    await conn.lpush("queue", ["task1", "task2"])
    head = await conn.lpop("queue")

    # -- Sorted Sets --
    await conn.zadd("leaderboard", {"alice": 1500.0, "bob": 1200.0})
    score = await conn.zscore("leaderboard", "alice")

    # -- JSON --
    await conn.json_set("product:1", "$", '{"name":"Widget","price":9.99}')
    name = await conn.json_get("product:1", "$.name")

    # -- Bulk Operations --
    await conn.mset({"k1": "v1", "k2": "v2"})
    vals = await conn.mget(["k1", "k2", "missing"])       # ["v1", "v2", None]

    # -- Consistency Levels --
    await conn.set("fast", "val", consistency=Consistency.EVENTUAL)
    await conn.set("safe", "val", consistency=Consistency.QUORUM)

    # -- Admin (admin only) --
    await conn.user_create("alice", "pass123", role="readwrite")
    await conn.user_role("alice", "readonly")
    await conn.user_grant("alice", db_id=1)
    users = await conn.user_list()

    # -- Error Handling --
    from mneme import MnemeError, KeyNotFound, WrongType, TokenExpired
    try:
        await conn.get("nonexistent")
    except KeyNotFound:
        print("Key does not exist")
    except TokenExpired:
        await conn.auth("admin", "secret")
    except MnemeError as e:
        print(f"MnemeCache error: {e}")
...
### 16.3 Node.js / TypeScript

Install: `npm install @mneme/client`

```typescript
import { MnemePool, Consistency, MnemeError } from '@mneme/client';

async function main() {
  const pool = await MnemePool.create({
    addr: '10.0.0.1:6379',
    caCert: '/etc/mneme/ca.crt',
  ...
  // -- Strings --
  await conn.set('greeting', 'hello world');
  const val = await conn.get('greeting');
  ...
  // -- Hashes --
  await conn.hset('user:1001', { name: 'Alice', email: 'alice@example.com' });
  const name = await conn.hget('user:1001', 'name');

  // -- Lists --
  await conn.lpush('queue', ['task1', 'task2']);
  const head = await conn.lpop('queue');

  // -- Sorted Sets --
  await conn.zadd('leaderboard', [
    { member: 'alice', score: 1500.0 },
    { member: 'bob', score: 1200.0 },
  ]);
  const score = await conn.zscore('leaderboard', 'alice');

  // -- JSON --
  await conn.jsonSet('product:1', '$', '{"name":"Widget","price":9.99}');
  const jval = await conn.jsonGet('product:1', '$.name');

  // -- Bulk Operations --
  await conn.mset({ k1: 'v1', k2: 'v2' });
  const vals = await conn.mget(['k1', 'k2', 'missing']);
  ...
  // -- Admin --
  await conn.userCreate('alice', 'pass123', { role: 'readwrite' });
  const userList = await conn.userList();

  // -- Error Handling --
  try {
    await conn.get('nonexistent');
  } catch (err) {
    if (err instanceof MnemeError) {
      switch (err.code) {
        case 'KEY_NOT_FOUND':   break;
        case 'WRONG_TYPE':      break;
        case 'TOKEN_EXPIRED':   await conn.auth('admin', 'secret'); break;
        default:                console.error('error:', err);
      }
    }
  }
...
func main() {
    ...
    pool, err := mneme.NewPool(mneme.PoolConfig{
        Addr:           "10.0.0.1:6379",
        CACert:         "/etc/mneme/ca.crt",
        ServerName:     "mneme.local",
        Token:          "eyJ...",
    ...
    // -- Strings --
    _ = conn.Set(ctx, "greeting", []byte("hello"), 0, mneme.Quorum)
    val, _ := conn.Get(ctx, "greeting")
    _ = conn.Del(ctx, "greeting")

    // -- Counters --
    n, _ := conn.Incr(ctx, "page:views")
    n, _ = conn.IncrBy(ctx, "page:views", 10)

    // -- Hashes --
    _ = conn.HSet(ctx, "user:1001", map[string]string{"name": "Alice"})
    name, _ := conn.HGet(ctx, "user:1001", "name")

    // -- Lists --
    _ = conn.LPush(ctx, "queue", "task1", "task2")
    head, _ := conn.LPop(ctx, "queue")

    // -- Sorted Sets --
    _ = conn.ZAdd(ctx, "lb", mneme.Quorum,
        mneme.ZMember{Score: 1500.0, Member: "alice"},
    )
    score, _ := conn.ZScore(ctx, "lb", "alice")

    // -- JSON --
    _ = conn.JSONSet(ctx, "product:1", "$", `{"name":"Widget"}`)
    jval, _ := conn.JSONGet(ctx, "product:1", "$.name")

    // -- Bulk --
    _ = conn.MSet(ctx, map[string]string{"k1": "v1", "k2": "v2"}, 0)
    vals, _ := conn.MGet(ctx, "k1", "k2")

    // -- Consistency --
    _ = conn.Set(ctx, "fast", []byte("v"), 0, mneme.Eventual)
    _ = conn.Set(ctx, "safe", []byte("v"), 0, mneme.Quorum)

    // -- Error Handling --
    _, err = conn.Get(ctx, "nonexistent")
    if err != nil {
        switch {
        case mneme.IsKeyNotFound(err):  fmt.Println("not found")
        case mneme.IsTokenExpired(err): conn.Auth(ctx, "admin", "secret")
        default:                        log.Printf("error: %v", err)
        }
    }

    _ = val; _ = n; _ = name; _ = head; _ = score; _ = jval; _ = vals
}
```
...
```xml
<dependency>
    <groupId>io.mneme</groupId>
    <artifactId>mneme-client</artifactId>
...
public class MnemeExample {
    public static void main(String[] args) throws Exception {
        MnemePool pool = MnemePool.builder()
            .addr("10.0.0.1:6379")
            .caCert("/etc/mneme/ca.crt")
            .serverName("mneme.local")
            .token("eyJ...")
        ...
        try (MnemeConn conn = pool.acquire()) {
            // -- Strings --
            conn.set("greeting", "hello world");
            String val = conn.get("greeting");

            // -- Counters --
            long n = conn.incr("page:views");
            n = conn.incrby("page:views", 10);

            // -- Hashes --
            conn.hset("user:1001", Map.of("name", "Alice"));
            String name = conn.hget("user:1001", "name");

            // -- Lists --
            conn.lpush("queue", "task1", "task2");
            String head = conn.lpop("queue");

            // -- Sorted Sets --
            conn.zadd("lb", Consistency.QUORUM,
                Map.of("alice", 1500.0, "bob", 1200.0));
            Double score = conn.zscore("lb", "alice");

            // -- JSON --
            conn.jsonSet("product:1", "$", "{\"name\":\"Widget\"}");
            String jval = conn.jsonGet("product:1", "$.name");

            // -- Bulk --
            conn.mset(Map.of("k1", "v1", "k2", "v2"));
            List<Optional<String>> vals = conn.mget("k1", "k2");

            // -- Consistency --
            conn.set("fast", "v", 0, Consistency.EVENTUAL);
            conn.set("safe", "v", 0, Consistency.QUORUM);
        }

        // -- Error Handling --
        try (MnemeConn conn = pool.acquire()) {
            conn.get("nonexistent");
        } catch (MnemeException e) {
            switch (e.getErrorCode()) {
                case KEY_NOT_FOUND  -> System.out.println("not found");
                case TOKEN_EXPIRED  -> pool.reauth("admin", "secret");
                default             -> System.err.println("error: " + e);
            }
        }
